...
import pointnet_util as pointnet2_utils

import torch.nn as nn
import torch.nn.functional as F
import torch.nn.functional as func
from pointnet_util import PointNetSetAbstractionMsg,PointNetFeaturePropagation,farthest_point_sample
import torch
...
class GAPConv(nn.Module):
    ...
    def forward(self,xyz, feat): #(B,N,C) to #(B,N,C'), where C is point dimensionality and C' is geometric features
        ...
        affineFeat = affineFeat.view((B,N,F))*func.sigmoid(self.omega)+feat.transpose(1,2)                        
...
class GAPBlock(nn.Module):
    ...
    def __init__(self,n_points, n_feat):
        ...
        for i in range(self.moduleCount):
            self.GCN_list.append(GAPConv(n_points,min(n_points,16),n_feat))
        self.bn = nn.BatchNorm1d(n_feat)
...
class PointMLP(nn.Module):
    def __init__(self, num_classes=3,points=600, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        for i in range(len(pre_blocks)):
            ...
            self.GCN_list.append(GAPBlock(anchor_points,out_channel))
            # pre_block_module = PreExtraction(last_channel, out_channel, pre_block_num, groups=groups,
            #                                  res_expansion=res_expansion,
            #                                  bias=bias, activation=activation, use_xyz=use_xyz)
...
def GAPGCN(num_classes=3, **kwargs) -> PointMLP:
