...
from data import ModelNet40, Kumamoto
from torch.optim.lr_scheduler import CosineAnnealingLR
import sklearn.metrics as metrics
import numpy as np
import provider

def augdata(points,hard=True):
    #points = interpolate(points,640)
...
def parse_args():
    ...
    parser.add_argument('--min_lr', default=0.000, type=float, help='min lr')
...
def main():
    ...
    if not os.path.isfile(os.path.join(args.checkpoint, "best_checkpoint.pth")):
        ...
        state_d = checkpoint['net']
        new_d = dict()
        for key in state_d.keys():
            new_d[key.replace('module.','')] = state_d[key]
    ...
    elif (args.dataset=="Kumamoto"):
        train_loader = DataLoader(Kumamoto(partition='train', num_points=args.num_points), num_workers=args.workers,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(Kumamoto(partition='test', num_points=args.num_points), num_workers=args.workers,
                             batch_size=args.batch_size // 2, shuffle=False, drop_last=False)

    optimizer = torch.optim.SGD(net.parameters(), lr=args.learning_rate, momentum=0.9, weight_decay=0)
