...
def cal_loss(pred, gold, smoothing=True):
    ...
    #weight = torch.tensor([0.4,1]).cuda()

    if smoothing and pred.size(1)!=2:
        eps = 0.2
        n_class = pred.size(1)
        ...
        #print(log_prb.shape,weight[gold].shape)
        
        loss = (-(one_hot * log_prb)).sum(dim=1).mean()
        #print(loss)
    else:
        #print("hello")
        loss = F.cross_entropy(pred, gold, reduction='mean',weight = torch.tensor((0.5,1)).cuda())
