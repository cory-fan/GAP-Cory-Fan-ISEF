...
class GAPConv(nn.Module):
    
    def getGroup(self,xyz, k, feat):
        ...
        group = group.reshape(B,K,-1) #shaped this way for ease of downstream convolution
        return group,idx
    ...
    def __init__(self,n_points, k_value, n_feat):
        ...
        self.affine = nn.Parameter(torch.ones(self.k).float())
        self.omega = nn.Parameter(torch.tensor(0).float())
    ...
    def forward(self,xyz, feat): #(B,N,C) to #(B,N,C'), where C is point dimensionality and C' is geometric features
        ...
        affineFeat,idx = self.getGroup(xyz,self.k,feat)
        affineFac = func.softmax(self.affine,dim=0)
        affineFeat = affineFac.view(1,-1,1)*(affineFeat-feat.unsqueeze(1).reshape(B,1,N*F))
        affineFeat = affineFeat.sum(dim=1)
        affineFeat = affineFeat.view((B,N,F))*func.tanh(self.omega)+feat.transpose(1,2)                        
...
class GAPBlock(nn.Module):
    ...
    def __init__(self,n_points, n_feat):
        ...
        self.nfeat = n_feat
        self.moduleCount = 3
        self.attentionVec = nn.Parameter(torch.ones(self.moduleCount,1,1,1).float())
        self.GCN_list = nn.ModuleList()
        for i in range(self.moduleCount):
            self.GCN_list.append(GAPConv(n_points,min(n_points,8),n_feat))
        self.bn = nn.BatchNorm1d(n_feat)
    ...
    def forward(self,xyz,feat):
        ...
        featlist = []
        for i in range(self.moduleCount):
            x = self.GCN_list[i](xyz,x)
            featlist.append(x)
                
        x = torch.stack(featlist,dim=0)*func.softmax(self.attentionVec,dim=0)
        x = self.bn(x.sum(dim=0))
...
class LaplacianUnit(nn.Module):
    ...
    def forward(self,xyz, feat): #(B,N,C) to #(B,N,C'), where C is point dimensionality and C' is geometric features
        ...
        K = self.k
        affineFeat,idx = self.getGroup(xyz,self.k,feat)
        affineFeat = affineFeat-feat.unsqueeze(1).reshape(B,1,N*F)
        affineFeat = affineFeat.mean(dim=1).view(B,N,F)
        affineFeat = self.bn(self.rl(self.fc(affineFeat)).transpose(1,2))
        return affineFeat+feat

class Model(nn.Module):
    def __init__(self, points=600, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.laplacianUnit = LaplacianUnit(2048,8,3)
        
        self.useLaplacian = True
        ...
        for i in range(len(pre_blocks)):
            ...
            self.lap_blocks_list.append(GAPBlock(anchor_points,out_channel))
    ...
    def forward(self, x):
        ...
        #print(lpfeat.shape)
        batch_size, _, _ = x.size()
        #x = torch.concat((x,lpfeat),dim=1)
        x2 = self.embedding2(self.laplacianUnit(x,x))
        x = self.embedding(x)+x2  # B,D,N
        for i in range(self.stages):
            ...
            initialx = x
            x = self.lap_blocks_list[i](xyz,x)
            #print((x-initialx).sum())
...
def GAPGCN(num_classes=40, **kwargs) -> Model:
    print(num_classes)
