...
def load_data(partition, fpath):
    download()
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(BASE_DIR, 'data')
    all_data = []
    all_label = []
    for h5_name in glob.glob(os.path.join(DATA_DIR, fpath, 'ply_data_%s*.h5'%partition)):
...
class ModelNet40(Dataset):
    def __init__(self, num_points, partition='train'):
        self.data, self.label = load_data(partition,'modelnet40_ply_hdf5_2048')
...
class Kumamoto(Dataset):
    def __init__(self, num_points, partition='train'):
        print("Kumamoto dataset")
        self.data, self.label = load_data(partition,'kumamoto')
        self.num_points = num_points
        self.partition = partition        
